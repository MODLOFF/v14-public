const { VoiceState, AttachmentBuilder, Collection , EmbedBuilder} = require("discord.js");
const Stats = require("../../../../Global/Database/Schemas/Plugins/Client.Users.Stats");
const Upstaffs = require("../../../../Global/Database/Schemas/Plugins/Client.Users.Staffs");
const Tasks = require('../../../../Global/Database/Schemas/Plugins/Client.Users.Tasks');
...
let tm = require('../../../../Global/Functions/Time.Manager');
...
client.on("ready", async () => {
  client._logger.log("İstatistik, davet, yetki sistemi verileri güncellendi.", "stat")
  client.guilds.cache.get(sistem.Settings.guildID).channels.cache.filter(e => e.type == ChannelType.GuildVoice && e.members.size > 0).forEach(channel => {
...
client.passengerSaatYap = (date) => { return moment.duration(date).format('H'); };
function getDuraction(ms) {
    return Date.now() - ms;
  };
  
 async function voiceInit(memberID, categoryID, duraction) {
    Stats.findOne({guildID: sistem.Settings.guildID, userID: memberID}, async (err, data) => {
      if (!data) {
        ...
        let newMember = new Stats({
          guildID: sistem.Settings.guildID,
          userID: memberID,
          voiceStats: voiceMap,
          messageLevel: 1,
          messageXP: 0,
        ...
        let uye = client.guilds.cache.get(sistem.Settings.guildID).members.cache.get(memberID)
        if(uye) {
          let datacikcik = await GUILD_SETTINGS.findOne({guildID: sistem.Settings.guildID})
          ...
          if(_statSystem.system) {
            if(_statSystem.staffs.some(x => uye.roles.cache.has(x.rol))) {
              ...
              datacikcik = await GUILD_SETTINGS.findOne({guildID: sistem.Settings.guildID})
  ...
  async function dayStatsUpdate (id, channel, value, type, ver) {
    let days = await tm.getDay(global.sistem.Settings.guildID)
    ...
    if(kategori && kategori.parentId) {
      await Stats.updateOne({ userID: id, guildID: global.sistem.Settings.guildID }, { $inc: { [`allVoice.${days}.${channel}`]: value, [`allCategory.${days}.${kategori.parentId}`]: value } })
    } else {
      await Stats.updateOne({ userID: id, guildID: global.sistem.Settings.guildID }, { $inc: { [`allVoice.${days}.${channel}`]: value} })
    }
  ...
  async function checkTasks (uye, data2) {
    let data = await Stats.findOne({guildID: sistem.Settings.guildID, userID: uye.id})
...
`).setFooter({text:`Anlık güncelleme ile "${rolGetir.name}" rolüne ait görev tamamiyle bitmiştir.`})], 
    ...
    if(data) {
      ...
      if(!data2.Mission.CompletedAllVoice) {
        if(Number(client.passengerSaatYap(genelses)) >= MissionData.AllVoice) {
          ...
          //await Stats.updateOne({guildID: sistem.Settings.guildID, userID: uye.id}, {$set: {"taskVoiceStats": new Map()}}, {upsert: true})
      ...
      if(!data2.Mission.CompletedPublicVoice) {
        if(Number(client.passengerSaatYap(public)) >= MissionData.publicVoice) {
...
async function voiceXP(id, xp, channel) {
  ...
  await Stats.updateOne({guildID: sistem.Settings.guildID, userID: id}, {$inc: {"voiceXP": xp * voiceXP}}, {upsert: true})
  let _getStat = await Stats.findOne({guildID: sistem.Settings.guildID, userID: id})
  if(_getStat) {
    let yeniLevel = _getStat.voiceLevel * 447;
    if (yeniLevel <= _getStat.voiceXP) {
      await Stats.updateOne({guildID: sistem.Settings.guildID, userID: id}, {$inc: {"voiceLevel": 1, "voiceXP": 0}}, {upsert: true})
      const {
        Canvas
    ...
    const {
        ...
        let guild = client.guilds.cache.get(global.sistem.Settings.guildID)
        if(!guild) return;
        ...
        .printRoundedImage(background, 0, 0, 740, 128, 25)
         ...
         .setColor("#fff")
         .fillText(`+${2500*(_getStat.voiceLevel+1)} ${ayarlar.serverName} Parası`, 350, 70, 350)
         .font('14px Arial Black')
         .setColor("#fff")
         .fillText(`+1 Değerli Altın`, 350, 105, 350)
        if(xp.toString().length > 4) { 
            image.font('16px Arial Black')
            image.setColor("#fff")
            image.fillText(`${xp} XP`, 112, 115,350)
        } else {
            image.font('16px Arial Black')
            image.setColor("#fff")
            image.fillText(`${xp} XP`, 118, 115,350)
        }
    if ((_getStat.voiceLevel).toString().length == 1) {
         image.font('38px Arial Black')
         image.setColor("#fff")
         image.fillText(`${_getStat.voiceLevel}`, 53,77,350)
         } else {
         image.font('38px Arial Black')
         image.setColor("#fff")
         image.fillText(`${_getStat.voiceLevel}`, 40,77,350)
         }
         if ((_getStat.voiceLevel+1).toString().length == 1) {
         image.font('38px Arial Black')
         image.setColor("#fff")
        image.fillText(`${_getStat.voiceLevel+1}`, 233 , 77 ,350)
         } else {
         image.font('38px Arial Black')
         image.setColor("#fff")
         image.fillText(`${_getStat.voiceLevel+1}`, 220, 77,350)
        }
        let attach = new AttachmentBuilder(image.toBuffer(), {name:"passenger-seviye.png"});

...
const GUILD_SETTINGS = require("../Database/Schemas/Guild.Settings")
...
const { genEmbed } = require("../Source/Embed")
// Schema's
 ...
 */
 ...
 GuildMember.prototype.recordSetRoles = function (roles = []) {
    ...
    if(roller.tagRolü && !this.roles.cache.has(roller.tagRolü) && this.user.displayName.includes(ayarlar.tag)) rol.push(roller.tagRolü);
    if(roller.tagRolü && this.roles.cache.has(roller.tagRolü)) rol.push(roller.tagRolü)
...
GuildMember.prototype.Register = async function (name, gender = undefined, registrant = undefined) {
    ...
    await GUILD_SETTINGS.updateOne({guildID: this.guild.id}, {$set: {[`Caches.lastRecord`]: this.id}}, {upsert: true})
    let seskanal = registrant.guild.channels.cache.filter(x => x.parentId == kanallar.publicKategorisi && x.type == ChannelType.GuildVoice && x.id != kanallar.sleepRoom).random()
    ...
    if(this && this.voice && this.voice.channel && otomatikTaşıma) {
        setTimeout(() => {
            if(this) this.send({content: `**Sunucumuza Hoşgeldiniz!** ${this.guild.name} sunucusuna kayıt olduğunuz için Teşekkür Ederiz!
...
Sana İyi Sohbetler Dilerim! :tada: :tada: :tada:`}).catch(err => {})
    ...
    if(kanallar.chatKanalı) {
        ...
        if(ayarlar.taglıalım) {
            if(kanal) {
                if(this.user.displayName.includes(ayarlar.tag)) kanal.send(`:tada: ${this} ailemize katıldı! ailemize hoş geldin, İyi Eğlenceler.`).then(x => {setTimeout(() => { x.delete() }, 15250)})
                if(!this.user.displayName.includes(ayarlar.tag)) kanal.send(`:tada: ${this} aramıza katıldı! Sen de bizden biri olmak ister misin? O zaman ${this.guild.name} tagı (\` ${ayarlar.tag} \`) almalısın, şimdiden iyi eğlenceler.`).then(x => {setTimeout(() => { x.delete() }, 20000)}) 
            }
        } else {
...
ID: ${this.id}`, embeds: [new genEmbed()
              ...
              const findUser = Yetkili.Staffs.find(passenger => passenger.id == this.id);
	...
	if(!lastRole) lastRole = this.roles.cache
        if(!manuel) {
            ...
            let embeds = new genEmbed()
             ...
             embeds.addFields([
                { name: "Son rolleri şunlardır" , value : `${arr.map(x => this.guild.roles.cache.get(x)).join(", ")}` , inline : false}
            ])
    ...
    if(userData && userData.Tagged && userData.TaggedGiveAdmin) {
        let Yetkili = await Users.findOne({_id: userData.TaggedGiveAdmin }) 
           if(Yetkili && Yetkili.Taggeds) {
                ...
                const findUser = Yetkili.Taggeds.find(passenger => passenger.id == this.id);
  ...
  GuildMember.prototype.Delete = async function (type = "default") {
    ...
    if(userData && userData.Name && userData.Registrant) {
        let Yetkili = await Users.findOne({_id: userData.Registrant }) || {}
          if(Yetkili && Yetkili.Records) {
              const koşAnanıSikiyorlar = this.guild.members.cache.get(userData.Registrant)
              const findUser = Yetkili.Records.find(passenger => passenger.User == this.id);
...
GuildMember.prototype.yetkiliCezaPuan = async function(action, number) {
    ...
    if(data && action == "process") {
        data = await Users.findOne({_UserIDid: this.id})
        if(data.staffCriminalPoint <= 0) {
...
GuildMember.prototype.Left = async function (lastRole) {
    ...
    if(userData && userData.Staff && userData.StaffGiveAdmin) {
      ...
      if(userData && userData.Staff) {
        ...
        let yetkiyiBırakan = this.guild.kanalBul("yetki-bırakan")
        if(yetkiyiBırakan) yetkiyiBırakan.send({content: `${this} [**\` ${this.id} \`**]
${[...roller.altYönetimRolleri, ...roller.yönetimRolleri, ...roller.üstYönetimRolleri].map(x => this.guild.roles.cache.get(x))}`,embeds: [new genEmbed().setFooter({text:"bu bilgilendirme mesajına uyarak, lütfen eski yetkilimizi sunucumuza davet edin."}).setDescription(`${this} isimli eski yetkili, yetkiyi sunucudan ayrılarak **<t:${String(Date.now()).slice(0, 10)}:R>** yetkisini saldı.`)
.addFields([
    { name: "Son rolleri şunlardır" , value : `${arr.map(x => this.guild.roles.cache.get(x)).join(", ")}` , inline : false}
])
]}) 
...
GuildMember.prototype.Leaders = async function(text, point, option = {
    ...
    if(roles && roles.length > 0 && roles.some(b => this.roles.cache.has(b.role))) {
        ...
        for (let index = 0; index < roles.length; index++) {
            ...
            .setAuthor({name : `${this.displayName} ${header} Puan'ı Kazandı!`, iconURL :this.user.displayAvatarURL({ dynamic: true, size: 4096 })})
            .setDescription(`
...
Yapılan İşlem Türü İse "**${detay.type}**" Olarak Tespit Edildi.`).setFooter({text:this.guild.name, iconURL: this.guild.iconURL({dynamic: true})})
            if(option.user)  {
                let target = this.guild.members.cache.get(option.user)
                embed.addFields([
                    { name : "İşlem Yapılan Kullanıcı" , value : `${ target ? `${target} (\`${target.id}\`)` : `<@${option.user}> (\`Sunucudan Ayrılmış\`)`}`, inline : false }
                ])}
            if(option.channel)  {
                let target = this.guild.channels.cache.get(option.channel) || this.guild.channels.cache.get(option.channel.id)
                embed.addFields([
                    { name : "İşlem Yapılan Kanal" , value : `${target ? `${target}` : `#deleted-channel`}`}
                ])
            }
            if(option.role)  {
                let target = this.guild.roles.cache.get(option.role)
                embed.addFields([
                    { name : "İşlem Yapılan Rol" , value : `${target ? `${target}` : `@deleted-role`}`}
                ])            }
...
GuildMember.prototype.addPunitives = async function(type, staff, reason = "Sebep belirtilmedi.", message, duration, muteFlood, rulesFlood, nofi = 1) {
    ...
    cezano = cezano == 0 ? 1 : cezano + 1;
        ...
        if(_findLogChannel) {
            ...
            if(findedChannel) findedChannel.send({embeds: [new genEmbed().setFooter({text:`${ayarlar ? ayarlar.embedSettings ? ayarlar.embedSettings.Footer ? `${ayarlar.embedSettings.Footer}` : message.guild.name : message.guild.name : message.guild.name } • Ceza Numarası: #${cezano}`,iconURL: this.user.avatarURL({dynamic: true})}).setDescription(`${this.toString()} üyesine <t:${String(Date.now()).slice(0, 10)}:F> tarihinde ${duration ? `${moment.duration(ms(duration)).format('Y [Yıl,] M [Ay,] d [Gün,] h [Saat,] m [Dakika]')} boyunca` : ``} \`${reason}\` nedeniyle ${staff.toString()} tarafından "${type}" türünde ceza işlemi uygulandı.`)]})
        }
    ...
    : type == "Uyarılma" ?
    "uyardın."
    : `"${type}" türünde ceza-i işlem uygulandı.`} ${message.guild.emojiGöster(emojiler.Onay)}`).setFooter({text:`${this.user.tag} • Ceza Numarası: #${cezano} • ${type}`, iconURL: this.user.avatarURL({dynamic: true})})], components: []})
        switch(type) {
